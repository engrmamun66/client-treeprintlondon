export default defineNuxtPlugin(nuxtApp => {
    ...
    nuxtApp.vueApp.directive('keep-only-digits', (el, bindings) => {
        if(bindings.value === undefined || bindings.value === true || (typeof bindings.value == 'object')){
            if(bindings.value === undefined) bindings.value = {};
            let element 
            ...
            let update_value = (event) => {
                event.stopImmediatePropagation();
                element.value = element?.value?.replace(/\D/g, '')
            };
            element.removeEventListener('input', update_value);    
            element.addEventListener('input', update_value);
        }
    });
    nuxtApp.vueApp.directive('keep-only-digits-with-dot', (el, bindings) => {
        if(bindings.value === undefined || bindings.value === true || (typeof bindings.value == 'object')){
            if(bindings.value === undefined) bindings.value = {};
            let element;
            if (el?.type === 'text' || el?.type === 'number' || el?.type === 'password') {
            ...
            let update_value = (event) => {
                event.stopImmediatePropagation();
                element.value = element?.value?.replace(/[^0-9.]/g, ''); // Allow digits and dot
    ...
    nuxtApp.vueApp.directive('increment-decrement-by-wheel', (el, bindings) => {
        if(bindings.value === undefined || bindings.value === true || (typeof bindings.value == 'object')){
            if(bindings.value === undefined) bindings.value = {};
            let element;
            if (el?.type === 'text' || el?.type === 'number' || el?.type === 'password') {
            ...
            let update_value = (event) => {
                try {
                    ...
                    console.warn('directive_errr::increment-decrement-by-wheel', error)
                }
                 
            };
        
            element.removeEventListener('wheel', update_value);
        
            element.addEventListener('wheel', update_value);
        }
    });
    ...
    nuxtApp.vueApp.directive('validation', {mounted:(el, bindings) => {
         ...
         * required:if-#status,#user_email ----------------| Note: Just run useNuxtApp().$emit('v-validation', 'bypass_errormessage'), when changed depended fields
         * 
        ...
        try {
            ...
            if (!label_element()) return console.error('lable tag not found for ' + field_name )
