<template>
    <div>
            ...
            <div class="ionic-admin-subheader-inner">
                ...
                <div class="ionic-subheader-rightside timeline-subheader-rightside">
                    ...
                    <div class="timelapes-clips-btn d-flex justify-content-between"> 
                        ...
                        <button
                            :tooltip="isHistoryMode ? 'Show live stream'  : 'Show history mode'"
                            @click="toggleIframeMode()"
                            class="ionic-btn ionic-theme-btn2 historylivestream-btn"
                            >
                            <i class="las la-exchange-alt size-sm cp mt-1"></i>
                        </button> 
                        <button
                        ...
                        <button 
                            ...
                            class="ionic-btn ionic-theme-btn2">
                            <i class="las la-cloud-download-alt"></i>Clips
                        </button>
                    </div>
        ...
        <div class="row">
            <div class="col-xl-4 col-lg-5 col-md-6" v-if="isCollapsedIframe">
                <div class="ionic-tab">
                    <ul>
                        <!-- <li>
                            <a class="tab-active">
                                                ...
                                                <div class="card-header-rightside" >
                                                    ...
                                                    <div class="camera-list-videobox">
                                                        ...
                                                        <div class="camera-box timeline-clip-box cp"  @click="isHistoryMode=true; loadVideoStream(clip, clip.camera_id, clip.start_timestamp);">
                                                            <img :src="clip.preview_image_url ? clip.preview_image_url : '/img/empty_img_small.jpg'" class="img-fluid" alt="camera img"/>
            ...
            <div :class="isCollapsedIframe ? 'col-xl-8 col-lg-7 col-md-6' : 'col-xl-12 col-lg-12 col-md-12 video-collapsfull-view' " @mouseenter="mouseentered=true" @mouseleave="mouseentered=false" >
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div v-else class="camera-box camera-large-box mb-4">
                                    <!-- <img src="/img/empty_img_small.jpg" class="img-fluid timeline-video-box-img" alt="camera img"/> -->
                                    <div class="camera-playicon">
                                        <i class="las la-play-circle"></i>
                                    </div>
        ...
        >
            <div class="modal-dialog modal-dialog-centered" @click.stop="closeClipsDownloadModal()">
                <div class="modal-content" @click.stop="false">
                <div class="modal-header">
                    <h1 class="modal-title fs-5">Download clips</h1>
                    <button
...
const shimmerEffect = ref(true)
const timeOut = null
let startDateForTimelapseModal = ref(H.formatDate(new Date(), "YYYY-MM-DD"));
let endDateForTimelapseModal = ref(H.formatDate(new Date(), "YYYY-MM-DD"));
onMounted(async () => {
  ...
  clearTimeout(timeOut)
    timeOut = setTimeout(()=>{
        if(!isHistoryMode.value) toggleIframeMode()
    }, 1000 * 60 * (useRuntimeConfig()?.public?.HIGHT_QUALITY_STREAM_TIMEOUT ?? 5) )
});
onBeforeUnmount(()=>{
    clearTimeout(timeOut)
...
async function getClipList(data=null){
  ...
  galleryStore.getVideoClips(payload);
}
...
function openClipsDownloadModal(){
    downloadClipsFormData.value.info.name =  `${downloadClipsFormData.value.type} ${galleryStore?.cameraDetails?.name} ${startDate.value.startDate} ${startTime.value.startTime}`
    downloadClipsFormData.value.deviceId = galleryStore?.cameraDetails?.een_camera_id
    showClipsDownloadModal.value = true;
}
function closeClipsDownloadModal(){
...
async function loadVideoStream(clip = null, cameraId, timestamp = null){
    ...
    if(clip){
        selectedParentClip.value = clip 
        currentTimeStamp.value = timestamp ? moment.utc(timestamp).local().format('MM-DD-YYYY HH:mm') : moment().local().format('MM-DD-YYYY HH:mm')
    }else{
...
function convertToCameraTimezone(time, timeFormat){
   return  moment.utc(time).tz(galleryStore?.cameraDetails?.timezone ?? 'US/Central').format(timeFormat)
}
...
watch(() => startDateTime.value.startDate, (newVal, oldVal)=>{
    if(newVal != oldVal){
        downloadClipsFormData.value.info.name =  `${downloadClipsFormData.value.type} ${galleryStore?.cameraDetails?.name} ${newVal}  ${startDateTime.value.startTime}`
...
watch(() => startDateTime.value.startTime, (newVal, oldVal)=>{
    if(newVal != oldVal){
        downloadClipsFormData.value.info.name =  `${downloadClipsFormData.value.type} ${galleryStore?.cameraDetails?.name} ${startDateTime.value.startDate}  ${newVal}`
...
function saveClipsDownloadForm(){
    ...
    galleryStore.downloadClipsFromEEN(downloadClipsFormData.value)
}
</script>
<style scoped>
.iframe-container {
