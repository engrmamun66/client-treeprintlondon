<template>
  <div>
      ...
      <div class="col-md-4">
        <div class="ionic-card ionic-card-textnumber">
          <div class="ionic-card-body">
            ...
            <h3>
              {{ galleryStore.cameraStatus?.company?.total_online_camera }}
            </h3>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div class="ionic-card ionic-card-textnumber">
          <div class="ionic-card-body">
            ...
            <h3>
              {{ galleryStore.cameraStatus?.company?.total_offline_camera }}
            </h3>
          </div>
        </div>
      </div>
    ...
    <template v-if="companyStore.mergedArray?.length"> 
      <section v-for="(site, index) in companyStore.mergedArray" :key="index">
        <div class="ionic-card">
          <div class="ionic-card-header">

            <a class="cp" @click="goToSiteCamerasPage(site)">
              <h4 class="fs-20 d-flex">
                {{ site?.title }} <img src="/img/siteicon.png" width="22px" height="22px" class="img-fluid" />
              </h4>
            </a>
          ...
          <div class="ionic-card-body">
            <div class="row">
              <div class="col-md-8">
                <div>
                  <div class="row">
                    <template v-for="(camera, cameraIndex) in site.cameras.filter(camera => camera.status === 1).slice(0, 4)" :key="cameraIndex">
                      <div class="col-md-3 dashboard-camera-column">
                      <template v-if="siteStore.cameraWithPreviewImage[camera.id]"> 
                        <div class="camera-box cp" @click.stop="navigateTo(`/vms/cameras/${camera?.id}/timeline`)" @mouseenter="isHovered = true; hoveredCamera = camera"
                        @mouseleave="isHovered = false; hoveredCamera = null">
                          ...
                          <div class="camera-bottom bottom-between">
                            ...
                            <div class="camera-bottom-right d-flex">
                              ...
                              <i class="bx bx-wifi wifi-icon" @contextmenu="log(camera.id, H.clone(camera))" :class="siteStore.cameraWithOfflineOnlineStatus.includes(camera?.id) ? 'is_online': 'is_offline'"></i>
                              <signalBar v-if="showSignalBars" @click.prevent.stop="log(camera?.cellular_signals?.rssi)" :signal="camera?.cellular_signals?.rssi" size="14px" color="limegreen" class="mx-1 mt-1" ></signalBar>
                            </div>
                          </div>
                        </div>
                      </template>
                    ...
                    <div class="d-flex justify-content-end">
                      <div @click="goToSiteCamerasPage(site)" v-if="site.cameras?.length > 4"
                        class="checkoutbox-btn-column text-center mx-2 d-flex justify-content-between">
                        <button class="ionic-btn ionic-theme-btn2">See more cameras <i
          ...
          <div class="col-md-12">
            ...
            >
              ...
              <div class="col-md-4">
                <div class="row">
                  <div class="col-md-6">
                    <div class="ionic-card onlineoffline-card ionic-card-textnumber">
                      <div class="ionic-card-body">
    ...
    <template v-else> 
      <section v-for="(site, index) in galleryStore?.list?.sites" :key="index">
        <div class="ionic-card">
          <div class="ionic-card-header">
...
async function asMounted(){
  ...
  if(!useCookie("header_company").value || useCookie("header_company").value == 'all' ){
    company_id = companyStore.companies[0]?.id;
    ...
    company_id = useCookie("header_company").value
  ...
  await galleryStore.getGallery(company_id);
  await galleryStore.getCameraStatus(company_id);
