            ...
            <div class="col-lg-6 auth-rightside">
              <div class="login-container">
                <div class="login-container-body">
                  <div class="login-heading mb-2">Forgotten Password?</div>
                  <div class="login-subheading mb-2">
                  ...
                  <div class="login-form">
                    <form @submit.prevent="submitLoginForm">
                      <div class="form-group">
                        ...
                        <span
                          v-if="
                            error.type === 422 && error.message?.email?.length
                          "
                          class="text-danger pt-3 ps-2 font-weight-normal"
                        >
                          {{
                            error.message?.email?.length
                              ? error.message.email.join(", ")
                              : ""
                          }}
                        </span>
                      </div>
                      <div
                        class="form-group mt-4 mb-0 d-flex justify-content-evenly align-items-center"
                      >
                        <button
                          type="button"
                        ...
                        >
                          Submit
                          <BtnLoader
                            v-if="H.isPendingAnyApi('Auth:emailReset')"
                            :show="true"
                            color="black"
                          />
                        </button>
...
const { login, emailReset } = useAuthStore();
const authStore = useAuthStore();
const { loading, error, emailConfirm } = storeToRefs(authStore);
...
async function submitLoginForm() {
  if (validateForm()) {
    await emailReset(userData.value);
  }
}
